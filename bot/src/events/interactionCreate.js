  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      if (client && client.processedInteractions && typeof client.processedInteractions.add === 'function') {
        client.processedInteractions.add(interaction.id);
        setTimeout(() => {
          try { client.processedInteractions.delete(interaction.id); } catch (_) {}
        }, client.DEDUPE_TTL_MS);
      }
    } catch (e) {
      console.error('[interactionCreate] Error during dedupe check:', e);
    ...
    if (interaction.isChatInputCommand && interaction.isChatInputCommand()) {
      ...
      try {
        ...
        console.error(error);
        await safeRespond({ content: 'コマンド実行中にエラーが発生しました。', flags: require('discord.js').MessageFlags.Ephemeral }).catch((e)=>{ console.error('Failed to send error response:', e); });
      }
      return;
    }
    ...
    if (interaction.isStringSelectMenu && interaction.isStringSelectMenu()) {
      ...
      if (interaction.customId === 'help_command_select') {
        const helpCommand = client.commands.get('help');
        if (helpCommand && typeof helpCommand.handleSelectMenu === 'function') {
          try { await helpCommand.handleSelectMenu(interaction); } catch (error) { console.error('ヘルプセレクトメニュー処理中にエラー:', error); await safeRespond({ content: 'メニュー処理でエラーが発生しました。', flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        }
      }
      return;
    }

    // ロールセレクト/チャンネルセレクトの処理
    if ((interaction.isRoleSelectMenu && interaction.isRoleSelectMenu()) || (interaction.isChannelSelectMenu && interaction.isChannelSelectMenu())) {
      ...
      if (interaction.customId.startsWith('channel_select_') || interaction.customId.startsWith('role_select_')) {
        ...
        if (guildSettings && typeof guildSettings.handleSelectMenuInteraction === 'function') {
          try { await guildSettings.handleSelectMenuInteraction(interaction); } catch (error) { console.error('ギルド設定セレクトメニュー処理中にエラー:', error); await safeRespond({ content: 'メニュー処理でエラーが発生しました。', flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        } else {
    ...
    if ((interaction.isModalSubmit && interaction.isModalSubmit()) || interaction.type === 5) {
      ...
      if (interaction.customId === 'default_title_modal' || interaction.customId === 'default_color_modal') {
        ...
        if (guildSettings && typeof guildSettings.handleModalSubmit === 'function') {
          try { await guildSettings.handleModalSubmit(interaction); } catch (error) { console.error('ギルド設定モーダル処理中にエラー:', error); await safeRespond({ content: `モーダル処理でエラー: ${error.message || error}`, flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        } else {
      ...
      if (interaction.customId.startsWith('editRecruitModal_')) {
        ...
        if (editRecruit && typeof editRecruit.handleEditModalSubmit === 'function') {
          try { await editRecruit.handleEditModalSubmit(interaction); } catch (error) { console.error('編集モーダル送信処理中にエラー:', error); await safeRespond({ content: `編集モーダル送信処理でエラー: ${error.message || error}`, flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        }
        return;
      }
      ...
      if (gameRecruit && typeof gameRecruit.handleModalSubmit === 'function') {
        try { await gameRecruit.handleModalSubmit(interaction); } catch (error) { console.error('モーダル送信処理中にエラー:', error); await safeRespond({ content: `モーダル送信処理でエラー: ${error.message || error}`, flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
      }
      return;
    }
    ...
    if (interaction.isButton && interaction.isButton()) {
      ...
      if (interaction.customId === 'grant_role_1420235531442196562') {
        ...
        if (!member) { await safeRespond({ content: 'メンバー情報が取得できませんでした。', flags: require('discord.js').MessageFlags.Ephemeral }); return; }
        if (member.roles.cache.has(roleId)) { await safeRespond({ content: 'すでにロールが付与されています。', flags: require('discord.js').MessageFlags.Ephemeral }); return; }
        try { await member.roles.add(roleId, 'ロール付与ボタンによる自動付与'); await safeRespond({ content: 'ロールを付与しました！', flags: require('discord.js').MessageFlags.Ephemeral }); } catch (e) { console.error('ロール付与エラー:', e); await safeRespond({ content: 'ロール付与に失敗しました。権限やロール位置を確認してください。', flags: require('discord.js').MessageFlags.Ephemeral }); }
        return;
      }
      // ロール剥奪ボタン
      if (interaction.customId === 'remove_role_1420235531442196562') {
        ...
        const member = interaction.guild?.members?.cache?.get(interaction.user.id);
        if (!member) { await safeRespond({ content: 'メンバー情報が取得できませんでした。', flags: require('discord.js').MessageFlags.Ephemeral }); return; }
        if (!member.roles.cache.has(roleId)) { await safeRespond({ content: 'ロールが付与されていません。', flags: require('discord.js').MessageFlags.Ephemeral }); return; }
        try { await member.roles.remove(roleId, 'ロール剥奪ボタンによる自動剥奪'); await safeRespond({ content: 'ロールを外しました。', flags: require('discord.js').MessageFlags.Ephemeral }); } catch (e) { console.error('ロール剥奪エラー:', e); await safeRespond({ content: 'ロールの剥奪に失敗しました。権限やロール位置を確認してください。', flags: require('discord.js').MessageFlags.Ephemeral }); }
        return;
      }
      ...
      if (interaction.customId === 'welcome_help') {
        const helpCommand = client.commands.get('help');
        if (helpCommand) {
          try { await helpCommand.execute(interaction); } catch (error) { console.error('ウェルカムヘルプボタン処理中にエラー:', error); await safeRespond({ content: 'ヘルプ表示でエラーが発生しました。', flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        }
        return;
      }
      ...
      if (interaction.customId === 'help_back') {
        ...
        if (helpCommand && typeof helpCommand.handleButton === 'function') {
          try { await helpCommand.handleButton(interaction); } catch (error) { console.error('ヘルプボタン処理中にエラー:', error); await safeRespond({ content: 'ボタン処理でエラーが発生しました。', flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
        }
        return;
      }
      ...
      if (gameRecruit && typeof gameRecruit.handleButton === 'function') {
        try { await gameRecruit.handleButton(interaction); } catch (error) { console.error('ボタン処理中にエラー:', error); await safeRespond({ content: `ボタン処理でエラー: ${error.message || error}`, flags: require('discord.js').MessageFlags.Ephemeral }).catch(()=>{}); }
      }
      return;
    }
  },
