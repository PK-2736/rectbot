...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    // Check for ownership challenge (skip authentication)
    let isOwnershipChallenge = false;
    const checkOwnershipChallenge = (raw) => {
      try {
        const payload = JSON.parse(raw);
        if (payload.content === 'test' || payload.content === 'tests') {
          isOwnershipChallenge = true;
        }
      } catch {}
    };
    ...
    if (req.headers['content-encoding'] === 'gzip') {
      zlib.gunzip(Buffer.concat(buf), (err, decompressed) => {
        if (!err) {
          ...
          checkOwnershipChallenge(raw);
        }
        processRequest(isOwnershipChallenge);
      });
    } else {
    ...
    function processRequest(isOwnershipChallenge) {
      if (!isOwnershipChallenge) {
      ...
      // Reset buf for processing
      buf = isOwnershipChallenge ? [] : buf;
      if (!isOwnershipChallenge) {
        ...
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ ok: true, challenge: 'accepted' }));
      }
    }
  });
});
