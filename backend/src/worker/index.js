...
<html lang="ja">
  ...
  const targetURL = 'https://api.recrubo.net' + url.pathname + url.search;
    ...
    // Cloudflare Worker logs -> forward to Loki (Worker -> Backend -> Loki)
    if (url.pathname === '/api/logs/cloudflare' && request.method === 'POST') {
      try {
        // Require service token (same pattern as /api/recruitment/push)
        if (!SERVICE_TOKEN) {
        ...
        if (!token || token !== SERVICE_TOKEN) {
          console.warn('[logs] Unauthorized attempt to push logs');
        ...
        if (!LOKI_PUSH_URL) {
          console.warn('[logs] LOKI_PUSH_URL not configured; dropping logs');
        ...
        // Normalize incoming logs into an array of { ts(ms)?, message, labels? }
        const entries = [];
        if (payload === null) {
          // try raw text
          ...
          // push the whole payload as one line
          entries.push({ message: JSON.stringify(payload) });
        }

        const streams = [];
        // Group entries by label-set (simple implementation: single stream)
        ...
        try {
          ...
          console.error('[logs] Loki push error:', err);
          return new Response(JSON.stringify({ ok: false, error: 'loki_push_error', detail: String(err) }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
        }
        ...
        console.error('[logs] Error handling cloudflare logs:', err);
      ...
      try {
        ...
        const serializedNotificationRoleId = (() => {
          ...
          try {
            ...
            console.warn('[finalize] Failed to serialize notification roles, falling back to first role:', e?.message || e);
          ...
          if (trimmed.startsWith('[') && trimmed.endsWith(']')) {
            try {
              ...
              if (Array.isArray(parsed)) {
                notificationRoles = parsed.filter(Boolean).map(String);
              }
            } catch (e) {
              console.warn('[guild-settings:get] Failed to parse notification_role_id JSON:', e?.message || e);
            }
          }
          if (notificationRoles.length === 0 && trimmed.length > 0) {
            notificationRoles = [trimmed];
          }
