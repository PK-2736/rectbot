...
function normalizeNotificationRoles(incomingSettings) {
  const roles = [];
...
function buildSupabasePayload(guildId, incomingSettings, serializedNotificationRoleId) {
  return {
    guild_id: guildId,
...
function buildSupabaseConfigStatus(env) {
  const supabaseRestUrl = resolveSupabaseRestUrl(env);
  const missing = [];
  if (!supabaseRestUrl) missing.push('SUPABASE_URL');
  if (!env.SUPABASE_SERVICE_ROLE_KEY) missing.push('SUPABASE_SERVICE_ROLE_KEY');
  return { supabaseRestUrl, missing };
}

function respondSupabaseMissing(missing, corsHeaders) {
...
function respondSupabaseUnreachable(reason, details, corsHeaders) {
  return respondJson({
    ok: true,
...
async function ensureSupabaseReachable(env, corsHeaders) {
  try {
    ...
    if (!pingOk) {
      ...
      return respondSupabaseUnreachable('Supabaseに接続できませんでした。', 'ping failed', corsHeaders);
    }
  } catch (e) {
    console.warn('[finalize] Supabase ping error:', e?.message || e);
    return respondSupabaseUnreachable('Supabase pingでエラーが発生しました。', e?.message || e, corsHeaders);
  }
  return null;
}
...
async function fetchExistingSettings(supabaseRestUrl, supabaseHeaders, guildId, corsHeaders) {
  ...
  if (!existingRes.ok) {
    ...
    return { response: respondSupabaseUnreachable(`Supabaseチェックに失敗しました: ${existingRes.status}`, errorText, corsHeaders) };
  }
  return { data: await existingRes.json() };
}

async function persistGuildSettings({ supabaseRestUrl, supabaseHeaders, guildId, incomingSettings, supabaseData, corsHeaders }) {
  const existing = await fetchExistingSettings(supabaseRestUrl, supabaseHeaders, guildId, corsHeaders);
  if (existing.response) return existing.response;

  const existingData = existing.data;
  let supaRes;
  if (Array.isArray(existingData) && existingData.length > 0) {
    const patchBody = buildPatchBody(incomingSettings, supabaseData);
    ...
    const supabaseBody = stripUndefinedValues(supabaseData);
  ...
  return respondJson({ ok: true, message: '設定が正常に保存されました' }, 200, corsHeaders);
}

function buildSettingsFromSupabase(data) {
  if (!Array.isArray(data) || data.length === 0) return null;
  const row = data[0];
  const notificationRoles = parseNotificationRoles(row.notification_role_id);
  const recruitChannels = Array.isArray(row.recruit_channel_ids)
    ? row.recruit_channel_ids.filter(Boolean).map(String)
    : [];

  return {
    recruit_channel: row.recruit_channel_id || (recruitChannels[0] || null),
    ...
    defaultTitle: row.default_title || '参加者募集',
...
async function fetchGuildSettings(env, guildId, corsHeaders) {
  const defaultSettings = buildDefaultSettings();
  const { supabaseRestUrl } = buildSupabaseConfigStatus(env);
  if (!supabaseRestUrl || !env.SUPABASE_SERVICE_ROLE_KEY) {
    ...
    return respondJson(defaultSettings, 200, corsHeaders);
  }
  ...
  const settings = buildSettingsFromSupabase(data) || defaultSettings;
  return respondJson(settings, 200, corsHeaders);
}
...
async function handleFinalize(request, env, corsHeaders) {
  try {
    ...
    if (!guildId) {
      return respondJson({ error: 'Guild ID required' }, 400, corsHeaders);
    }

    const normalizedNotificationRoles = normalizeNotificationRoles(incomingSettings);
    const serializedNotificationRoleId = serializeNotificationRoleId(normalizedNotificationRoles);
    const supabaseData = buildSupabasePayload(guildId, incomingSettings, serializedNotificationRoleId);
    const { supabaseRestUrl, missing } = buildSupabaseConfigStatus(env);
    if (missing.length > 0) {
      return respondSupabaseMissing(missing, corsHeaders);
    }

    const supabaseHeaders = buildSupabaseHeaders(env);
    const pingResponse = await ensureSupabaseReachable(env, corsHeaders);
    if (pingResponse) return pingResponse;

    return await persistGuildSettings({
      supabaseRestUrl,
      supabaseHeaders,
      guildId,
      incomingSettings,
    ...
    return respondJson({ error: 'Internal server error', details: error.message, timestamp: new Date().toISOString() }, 500, corsHeaders);
  }
}
...
async function handleGet(request, env, corsHeaders, url, _ctx) {
  try {
    ...
    return await fetchGuildSettings(env, guildId, corsHeaders);
  } catch (error) {
    console.error('Guild settings fetch error:', error);
    // best-effort: return defaults
    return respondJson(buildDefaultSettings(), 200, corsHeaders);
  }
}

// Router entry for guild-settings
export async function routeGuildSettings(context) {
